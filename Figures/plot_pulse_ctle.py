...
annots = loadmat('B12_Pulse_CTLE.mat')
pulse_ctle = annots['pulse_ctle']
pulse_ctle=pulse_ctle[:,0]

pulse_channel = annots['pulse_channel']
pulse_channel=pulse_channel[:,0]

pulse_channel_ctle = annots['pulse_channel_ctle']
pulse_channel_ctle=pulse_channel_ctle[:,0]
...
fig, ax = plt.subplots()
ax.plot(pulse_ui+0.35, pulse_ctle*1e3, color = 'red', linewidth = 3) 
ax.plot(pulse_ui+4, pulse_channel*1e3, color = 'blue', linewidth = 3) 
ax.plot(pulse_ui+4, pulse_channel_ctle*1e3, color = 'green', linewidth = 3) 
...
plt.legend(['CTLE Pulse Response','Output w/o Eq', 'Output w/i Eq'],prop = { "size": 16 ,'weight':'bold'}, loc ="upper right")
...
plt.savefig("plot_pulse_ctle.pdf")
